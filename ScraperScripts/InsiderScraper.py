...
filepathprefix = "/home/tomu/insiderscraper/logs/"
...
def scrapeInsiderData():
    try:
        yesterdaysDate = date.today() - timedelta(days=1)
        dateRef = str(yesterdaysDate.year) + "/"

        qtrRef = ""
        month = yesterdaysDate.month
        ...
        if month in (1,2,3):
            ...
            raise error('ERROR: Could not determine quarter.' + "\n")

        dateFormat = str(yesterdaysDate).split('-')
        url = feedURL + dateRef + qtrRef + "".join(dateFormat) + ".nc.tar.gz"

        requestCounter = 0
        logFile.write('Requestsing data from: ' + url + "\n")
        r = requests.get(url, headers=h)

        while r.status_code != 200 and requestCounter < 4:
            logFile.write('Request failed with return code: ' + str(r.status_code) + ' and response message: ' + "\n")
            logFile.write(r.text + "\n")
            logFile.write('Sleeping for 60 seconds...' + "\n")
            time.sleep(60)
            logFile.write('Starting request re-attempt ' + str(requestCounter + 1) + "\n")
            r = requests.get(url, headers=h)
            requestCounter += 1

        if requestCounter >= 4 and r.status_code != 200:
            logFile.write('ERROR: Unable to access the latest insider filing data. This log is for cases where the requet fails, and the lack of data causes the error log string to (potentially) fail.')
            raise error("ERROR: Multiple attempts to access the latest insider filing data from SEC.gov failed. Latest response code: " + str(r.status_code) + ' and response message: ' + r.text + "\n")
        return r
...
def parseInsiderFilings(r):
    tar = ''
    try:
        tar_like_obj = io.BytesIO(r.content)
        tar = tarfile.open(fileobj=tar_like_obj)
    except Exception as e:
        logError(True)
        ...
        try:
            if filingFile[-3:] == '.nc':
                ...
                for tradeEntryXML in tradeDataXML:
                    ...
                    if tradeEntryXML.find("transactionAmounts").find("transactionPricePerShare").find("value") is None:
                        logFile.write('\tContinuing - transaction price per share value is not available\n')
                        continue
...
# Remove existing items from the dailyinsiderdata container and upload the new data gathered from today's report
def manageAndUploadDailyData():
    try:
        logFile.write('\nAttempting to upload daily insider data\n')
        client = CosmosClient("insert_conn_str_here", "insert_key_here==")
        database = client.get_database_client('insiderdb')
        container = database.get_container_client('dailyinsiderdata')
        existingItems = container.read_all_items()
        for item in existingItems:
            container.delete_item(item["id"], item['Symbol'])

        for data in insiderData:
            container.upsert_item(insiderData[data])
        logFile.write('Upload of daily insider data is complete\n')
...
r = scrapeInsiderData()
parseInsiderFilings(r)
manageAndUploadDailyData()
manageAndUploadData()
uploadLogs()